...
LRESULT STG_Boss_Okayu_Last::Init(ID3D12Device* dev, const wchar_t* fileName, Unit_Side mySide, VTN_Item* item, bool add)
{
...
void STG_Boss_Okayu_Last::Draw(ID3D12GraphicsCommandList* cmdList)
{
...
void STG_Boss_Okayu_Last::DrawStatus(ID3D12GraphicsCommandList* cmdList)
{
	STG_Boss::DrawStatus(cmdList);
}

void STG_Boss_Okayu_Last::BossAI(STG_Unit** unit) {
...
void STG_Boss_Okayu_Last::BossDestroy()
{
	VTN_Image::Destroy();
}

void STG_Boss_Okayu_Last::BossRnadPatarn()
{
	int RMD = rand() % 5;
	switch (RMD)
	{
...
void STG_Boss_Okayu_Last::TimeOutAi(STG_Unit** unit)
{
	if (!isTimeOutStart)
	{
		isTimeOutStart = true;
...
void STG_Boss_Okayu_Last::BossRnadPatarn_Skiil()
{
	isSkill[0] = false;
	isSkill[1] = false;
	isSkill[2] = false;
