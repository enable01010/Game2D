...
void STG_Boss_Korone_Mid::BossAI(STG_Unit** unit)
{
	switch (State)
	{
	case BOSS_STATE::DOWN_FIRST:
		...
		{
			Bullet[(int)BulletType::HOMING]->SetBullet(Pos, Vector2(0,1),unit[0]->GetUnitStatus());
			BulletDelayTime = 0;
		}
	...
	case BOSS_STATE::ATTACK_ONE:
		...
		if (AttackTime > 50)
		{
			float radian = AttackTime * 3.14f * 0.0055555f * 10;
		...
		else
		{
			...
			float radian = AttackTime * 3.14f * 0.0055555f * 10;
			Bullet[(int)BulletType::YELLOW]->SetBullet(Pos + Vector2(cos(radian) * 10, sin(radian) * 10), Vector2(cos(radian) * 5, sin(radian) * 5));
...
void STG_Boss_Korone_Mid::BossDestroy()
{

}

void STG_Boss_Korone_Mid::Damage(int val)
{
...
void STG_Boss_Korone_Mid::TimeOutAi(STG_Unit** unit)
{
	Pos += Vector2(0, -3);
	if (Pos.y <= -450) {
		isClear = true;
