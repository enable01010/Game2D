...
void EasyMode_One::Init() {
	...
	//ボスの設定
	Unit[(int)UnitType::MID_BOSS]->Init(dev, L"img\\Boss\\Okayu.png");
	BossMid->SetPos(Vector2(0, -800));
	BossMid->SetScale(Vector2(0.1f));
	Unit[(int)UnitType::LAST_BOSS]->Init(dev, L"img\\Boss\\Okayu.png");
...
void  EasyMode_One::BackGroundMove()
{
	for (int i = 0; i < 2; i++)
	{
		playArea[i].Update();
...
void EasyMode_One::SpawnControl()
{
...
bool EasyMode_One::CheckEnemyClear()
{
...
void EasyMode_One::TextManagement()
{
	if (!isAllOutPut) {
...
void EasyMode_One::CheckGameOverAfter()
{
...
void EasyMode_One::CheckGameClearAfter()
{
